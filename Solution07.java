...
class Solution {
    public static void main(String[] args) {
        ...
        for (int[] x : theArray) {
         ...
         int[] point = { 2 , 0 };  
          ...
          int[][] clickedArr = click(theArray, point);
      
      
              for (int[] x : clickedArr) {
            for (int y : x) {
                System.out.print(y + " ");
            }
            System.out.println();
    ...
    public static int[][] click(int[][] theField, int[] clickedPoint) {
        ...
        while (checkList.size() > 0) {
            ...
            theRow = checkList.get(0)[0];
            theCol = checkList.get(0)[1];
            ...
            if (result[theRow][theCol] == 0) {
                ...
                for (int theCheckedRow = theRow - 1; theCheckedRow <= theRow + 1; theCheckedRow++) {
                    ...
                    for (int theCheckedCol = theCol - 1; theCheckedCol <= theCol + 1; theCheckedCol++) {
                            ...
                            System.out.println("checklist size: " + checkList.size() + " theRow: " + theRow + " theCol: " + theCol + " theCheckedRow: "+ theCheckedRow +  " theCheckedCol " + theCheckedCol);
                        
                          if(!(theCheckedRow == theRow && theCheckedCol == theCol) 
                            && theCheckedRow >= 0 && theCheckedRow < result.length
                           &&  theCheckedCol >= 0 && theCheckedCol < result[0].length
                          
                          ){
                          
                                System.out.println(checkList.size());
                          
                        int[] checkPoint = {theCheckedRow, theCheckedCol};
